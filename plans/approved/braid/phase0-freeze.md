...
| `Braid::preview_patch(&self, id) -> Result<Vec<FormatToken>, PatchError>` | **re-amended 2026-07-31 (Phase C close, reviewer-approved)** — pure `&self` working projection returning the id-optional working type | preview never mints: existing tokens retain their ids, synthesized tokens carry `id: None`; apply runs the same projection then admission mints and converts to resident `OwnedToken`s. "Preview equals apply" means semantic token content before admission, not equality of newly minted ids. Supersedes this table's earlier `&mut self`/`Vec<OwnedToken>` row |
