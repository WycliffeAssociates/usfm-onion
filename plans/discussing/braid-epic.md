...
1. As an editor user, I want typing and lint refreshes to remain synchronous in the webview, so
   that async lifecycle plumbing does not leak through the editor. With Tauri rust/ipc on first load for full corpora is desired/speed (editor concern)
...
5. A complete snapshot plus JS reconciliation is the v1 diff effect. There is no delta/tombstone
   protocol.
6. Interactive webview calls are synchronous wasm. (If the user chooses to postMessage asyc, that is their choice) Threaded wasm, SAB, COOP/COEP, and workers are
   absent.
...
`tests/lint_oracle.rs` is a normal snapshot assertion; do not describe it as ignored and never set
`BLESS=1`. Likewise do not set `UPDATE_GOLDEN=1`. Record pass/fail, duration, and any platform-only
...
| `braid` | complete ordered corpus; validation; current/baseline state; hashes; dirty books; mutation/lint lifecycle; native complete snapshots | byte layout, JS values, IO, scheduling, duplicate rule logic |
...
book-code tokens require `book_code`; marker-like tokens own marker metadata/attributes; other
kinds reject those payloads. If a single Rust struct makes those invariants too loose, use a
...
application, and lifecycle safety.
...
- `token_id_index[N]:u32` into a UTF-8 string dictionary;
- `sid_index[N]:u16` into the packed SID dictionary, with `0xffff = None`;
- `marker_descriptor_index[N]:u16`, with `0xffff = None`;
- sparse number records keyed by `token_idx`;
...
| 14 | flags | `u8` | exact/anchor-only, range, related, payload, fix, overflow |
...
marks one for deletion. Registry calls re-export core. The wasm crate defines no duplicate Token,
...
bulk native execution, but that is an application adapter, not a second braid lifecycle. Phase F
may adopt sync wasm in Tauri's webview first and defer resident native IPC. No plan step assumes
native Rayon automatically predicts webview wasm performance.
...
1. Land the separately blessed core declared-book lint context and mismatch rule from Gate 0;
   keep no-context stateless lint behavior unchanged.
...
5. Implement authoritative exact source bytes, per-book hashes, snapshot identity, and dirty-book
   stamps.
...
Gate: web and Tauri-webview paths pass identical lifecycle transcripts; initial load/edit/undo/
redo/format/fix/save/reopen findings match; no async coloring enters the interactive caller;
package import/golden tests pass; removed glue has no remaining callers; wasm size and warm-path
measurements are recorded without redefining correctness.
...
- Native Rayon is not evidence for webview wasm. Test bundler web, direct web, and Tauri webview.
...
- npm README documents buffer lifetime, sync behavior, `decodeView` vs materialize vs reconcile, cache
  validation, stable token-id requirement, and error handling.
...
8. **Native resident Tauri session:** bulk async IPC adapter over the same synchronous Rust Braid,
   only if product measurements justify the second hosting path.
...
- Still-separate plans: `lint-invalid-book-code.md`, `hygiene-caveats.md`, candidates/deferred
  output work, and consistency/monotonicity planning.
