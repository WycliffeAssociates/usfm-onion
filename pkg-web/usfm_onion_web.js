...
export function formatFlatTokenBatches(request) {
    const ret = wasm.formatFlatTokenBatches(request);
    var v1 = getArrayJsValueFromWasm0(ret[0], ret[1]).slice();
    wasm.__wbindgen_free(ret[0], ret[1] * 4, 4);
    return v1;
}
...
export function intoUsjLossless(document) {
    const ret = wasm.intoUsjLossless(document);
    return ret;
}
...
export function intoUsxLossless(request) {
    ...
    let deferred2_1;
    try {
        const ret = wasm.intoUsxLossless(request);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
...
export function intoUsxLosslessFromTokens(tokens) {
    ...
    try {
        const ptr0 = passArrayJsValueToWasm0(tokens, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.intoUsxLosslessFromTokens(ptr0, len0);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
...
export function lintFlatTokenBatches(request) {
    const ret = wasm.lintFlatTokenBatches(request);
    var v1 = getArrayJsValueFromWasm0(ret[0], ret[1]).slice();
    wasm.__wbindgen_free(ret[0], ret[1] * 4, 4);
    return v1;
}
