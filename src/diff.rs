use crate::handle::tokens;
use crate::parse::parse;
use crate::token::{FlatToken, TokenKind, TokenViewOptions};
#[cfg(feature = "rayon")]
use rayon::prelude::*;
...
pub fn build_sid_blocks<T: DiffableFlatToken>(
    ...
    while let Some(start) = next_block_start(tokens, index, options) {
        ...
        let block_id = if let Some(first_token_id) = tokens[start].id() {
        ...
        match change.tag {
            DiffStatus::Added => {
                for id in change.ids {
                    ...
                    let current_text_only = strip_usfm_markers_for_display(&current.text_full);
                    out.push(SidBlockDiff {
                        ...
                        current_text_only: current_text_only.clone(),
                        is_whitespace_change: false,
                        is_usfm_structure_change: current_text_only.is_empty(),
                    });
                }
            }
            DiffStatus::Deleted => {
                for id in change.ids {
                    ...
                    let original_text_only = strip_usfm_markers_for_display(&original.text_full);
                    out.push(SidBlockDiff {
                        block_id: id,
                        ...
                        original_text_only: original_text_only.clone(),
                        current_text_only: String::new(),
                        is_whitespace_change: false,
                        is_usfm_structure_change: original_text_only.is_empty(),
                    });
                }
            }
...
fn align_removed_added_chunk<T: DiffableFlatToken>(
    ...
    let original_slice = &original_tokens[original_start..original_start.saturating_add(original_len)];
...
fn token_kind_key(kind: &TokenKind) -> &'static str {
    match kind {
        TokenKind::HorizontalWhitespace => "horizontalWhitespace",
        TokenKind::VerticalWhitespace => "verticalWhitespace",
...
pub fn apply_revert_by_block_id<T: DiffableFlatToken>(
    ...
    let baseline = baseline_by_id.get(diff_block_id).copied().or_else(|| {
        infer_sid_match_block(diff_block_id, &baseline_blocks)
    });
    let current = current_by_id.get(diff_block_id).copied().or_else(|| {
        infer_sid_match_block(diff_block_id, &current_blocks)
    });
...
pub fn diff_usfm_sources(
    ...
    let current_tokens = tokens(&current, *token_view);
    diff_chapter_token_streams(&baseline_tokens, &current_tokens, build_options)
}

#[cfg(feature = "rayon")]
pub fn diff_usfm_sources_parallel(
    baseline_usfm: &str,
    current_usfm: &str,
    token_view: &TokenViewOptions,
    build_options: &BuildSidBlocksOptions,
    ...
    let baseline_blocks = build_sid_blocks(&baseline_tokens, build_options);
    let current_blocks = build_sid_blocks(&current_tokens, build_options);
    let diffs = diff_sid_blocks(&baseline_blocks, &current_blocks);
...
pub fn diff_usfm_sources_by_chapter(
    baseline_usfm: &str,
    current_usfm: &str,
    token_view: &TokenViewOptions,
    build_options: &BuildSidBlocksOptions,
    ...
    let baseline_groups =
        group_tokens_by_book_and_chapter(&baseline_tokens, baseline.book_code().unwrap_or("unknown"));
    let current_groups =
        group_tokens_by_book_and_chapter(&current_tokens, current.book_code().unwrap_or("unknown"));
...
#[cfg(feature = "rayon")]
pub fn diff_usfm_sources_by_chapter_parallel(
    baseline_usfm: &str,
    current_usfm: &str,
    token_view: &TokenViewOptions,
    build_options: &BuildSidBlocksOptions,
    ...
    let mut all_keys = BTreeSet::<(String, u32)>::new();
    for (book, chapters) in &baseline_groups {
        for chapter in chapters.keys() {
            all_keys.insert((book.clone(), *chapter));
        }
    }
    for (book, chapters) in &current_groups {
        for chapter in chapters.keys() {
            all_keys.insert((book.clone(), *chapter));
        }
    }
    ...
    replace_many_chapter_diffs_in_map(&DiffsByChapterMap::new(), &entries)
}
...
fn diff_sequences(original_keys: &[String], current_keys: &[String]) -> Vec<SequenceChange> {
    let original = original_keys
        .iter()
        .map(|id| format!("{id}\n"))
        .collect::<String>();
...
fn build_modified_diff(
    ...
    let original_text_only = strip_usfm_markers_for_display(&original_text);
    let current_text_only = strip_usfm_markers_for_display(&current_text);
    let is_whitespace_change = strip_all_whitespace(&original_text) == strip_all_whitespace(&current_text);
    let is_usfm_structure_change = !is_whitespace_change
        && strip_all_whitespace(&original_text_only) == strip_all_whitespace(&current_text_only);
...
fn coalesce_delete_add_pairs(diffs: Vec<SidBlockDiff>) -> Vec<SidBlockDiff> {
    ...
    for (sid, deleted_indexes) in deleted_by_sid {
        ...
        for pair in 0..pair_count {
            ...
            let (Some(original), Some(current)) = (deleted.original.clone(), added.current.clone()) else {
                continue;
            };
...
fn group_tokens_by_book_and_chapter<T: DiffableFlatToken>(
    ...
    for token in tokens {
        let sid = token.sid().unwrap_or_default();
...
mod tests {
    use super::*;
    use crate::token::{FlatToken, TokenKind, WhitespacePolicy};
    ...
    fn uses_first_token_id_for_stable_block_ids() {
        let tokens = vec![t("GEN 1:1", "A", Some("tok-aaa")), t("GEN 1:1", "B", Some("tok-bbb"))];
        let blocks = build_sid_blocks(&tokens, &BuildSidBlocksOptions::default());
        assert_eq!(blocks[0].block_id, "GEN 1:1::tok-aaa");
    }

    #[test]
    fn treats_non_consecutive_repeated_sid_as_distinct_blocks() {
        let tokens = vec![
            t("GEN 1:1", "A", Some("a")),
            t("GEN 1:2", "B", Some("b")),
            t("GEN 1:1", "C", Some("c")),
        ...
        assert_eq!(blocks[0].block_id, "GEN 1:1::a");
        assert_eq!(blocks[2].block_id, "GEN 1:1::c");
    }

    #[test]
    fn marks_modified_blocks_and_reverts_by_block_id() {
        let baseline = vec![
            t("GEN 1:1", "Hello ", Some("a")),
            t("GEN 1:1", "world", Some("b")),
            t("GEN 1:2", "Ok", Some("c")),
        ];
        let current = vec![
            t("GEN 1:1", "Hello ", Some("a")),
            t("GEN 1:1", "wurld", Some("b")),
            t("GEN 1:2", "Ok", Some("c")),
        ];
        ...
        let modified = diffs.iter().find(|diff| diff.status == DiffStatus::Modified).unwrap();
        ...
        assert_eq!(
            reverted.iter().map(|token| token.text.as_str()).collect::<String>(),
            "Hello worldOk"
        );
    }
    ...
    #[test]
    fn coalesces_non_adjacent_id_drift_and_keeps_unchanged_text_unchanged() {
        let baseline = vec![
            t("ISA 33:9", "Verse 9", Some("orig-9")),
            t("ISA 33:10", "Verse 10", Some("orig-10")),
        ];
        let current = vec![
            t("ISA 33:9", "Verse 9 edited", Some("new-9")),
            t("ISA 33:10", "Verse 10", Some("new-10")),
        ];
        ...
        assert_eq!(
            diffs
                .iter()
                .find(|diff| diff.semantic_sid == "ISA 33:9")
                .unwrap()
                .status,
            DiffStatus::Modified
        );
        assert_eq!(
            diffs
                .iter()
                .find(|diff| diff.semantic_sid == "ISA 33:10")
                .unwrap()
                .status,
            DiffStatus::Unchanged
        );
        assert!(!diffs.iter().any(|diff| diff.status == DiffStatus::Added));
        assert!(!diffs.iter().any(|diff| diff.status == DiffStatus::Deleted));
    }

    #[test]
    fn revert_deleted_block_inserts_after_previous_anchor() {
        let baseline = vec![
            t("GEN 1:1", "A", Some("a")),
            t("GEN 1:2", "B", Some("b")),
            t("GEN 1:3", "C", Some("c")),
        ];
        let current = vec![t("GEN 1:1", "A", Some("a")), t("GEN 1:3", "C", Some("c"))];
    ...
    #[test]
    fn revert_added_block_removes_current_slice() {
        let baseline = vec![t("GEN 1:1", "A", Some("a")), t("GEN 1:3", "C", Some("c"))];
        let current = vec![
            t("GEN 1:1", "A", Some("a")),
            t("GEN 1:2", "B", Some("b")),
            t("GEN 1:3", "C", Some("c")),
    ...
    #[test]
    fn supports_batch_revert_by_multiple_block_ids() {
        let baseline = vec![t("GEN 1:1", "A", Some("a")), t("GEN 1:3", "C", Some("c"))];
        let current = vec![
            t("GEN 1:1", "A1", Some("a")),
            t("GEN 1:2", "B", Some("b")),
        ...
        let reverted = apply_reverts_by_block_id(
            &["GEN 1:1::a".to_string(), "GEN 1:2::b".to_string(), "GEN 1:3::c".to_string()],
            &baseline,
            &current,
            &BuildSidBlocksOptions::default(),
        );
    ...
    fn detects_whitespace_only_changes() {
        ...
        let diff = diff_chapter_token_streams(
            &baseline,
            &current,
            &BuildSidBlocksOptions::default(),
        )
    ...
    #[test]
    fn detects_usfm_structure_only_changes() {
        let baseline = vec![t("GEN 1:1", "\\bd Alpha\\bd*", Some("a"))];
        let current = vec![t("GEN 1:1", "Alpha", Some("a"))];
        let diff = diff_chapter_token_streams(
            &baseline,
            &current,
            &BuildSidBlocksOptions::default(),
    ...
    #[test]
    fn supports_custom_token_types() {
        let baseline = vec![t("GEN 1:1", "Alpha", Some("a"))];
        let current = vec![t("GEN 1:1", "Beta", Some("a"))];
        let diffs = diff_chapter_token_streams(
            &baseline,
            &current,
            &BuildSidBlocksOptions::default(),
        ...
        assert_eq!(diffs.len(), 1);
        assert_eq!(diffs[0].current_tokens[0].text, "Beta");
    }

    #[test]
    fn emits_token_alignment_metadata_for_marker_number_and_text() {
        let baseline = vec![
            ft("REV 19:4", "m-v", TokenKind::Marker, Some("v"), "\\v "),
            ft("REV 19:4", "n-v", TokenKind::Number, None, " 4"),
            ft("REV 19:4", "t-v", TokenKind::Text, None, " old"),
        ];
        let current = vec![
            ft("REV 19:4", "m-v", TokenKind::Marker, Some("v"), "\\v"),
            ft("REV 19:4", "n-v", TokenKind::Number, None, "4"),
            ft("REV 19:4", "t-v", TokenKind::Text, None, " new"),
        ];
    ...
    #[test]
    fn deleted_diff_prefers_original_undo_side() {
        let baseline = vec![ft("GEN 1:1", "t-1", TokenKind::Text, None, "Alpha")];
        let current = vec![];
        let diff = diff_chapter_token_streams(
            &baseline,
            &current,
        ...
        assert_eq!(diff.undo_side, DiffUndoSide::Original);
    }

    #[test]
    fn chapter_diff_map_helpers_replace_and_prune() {
        let mut map = DiffsByChapterMap::new();
        map = replace_chapter_diffs_in_map(&map, "GEN", 1, vec![1usize]);
        assert_eq!(map.get("GEN").and_then(|chapters| chapters.get(&1)), Some(&vec![1]));

        map = replace_chapter_diffs_in_map(&map, "GEN", 1, Vec::<usize>::new());
        assert!(map.is_empty());
    }

    #[test]
    fn replace_many_and_flatten_diff_map_work() {
        let map = replace_many_chapter_diffs_in_map(
            &DiffsByChapterMap::new(),
            &[
                ("GEN".to_string(), 1, vec![1usize, 2]),
                ("EXO".to_string(), 2, vec![3usize]),
            ],
        );
        let flat = flatten_diff_map(&map);
        assert_eq!(flat, vec![3, 1, 2]);
    }

    #[test]
    fn diff_usfm_sources_by_chapter_groups_overlap_and_missing_chapters() {
        let baseline = "\\id GEN\n\\c 1\n\\p\n\\v 1 Alpha\n\\c 2\n\\p\n\\v 1 Beta\n";
        let current = "\\id GEN\n\\c 1\n\\p\n\\v 1 Alpha edited\n\\c 3\n\\p\n\\v 1 Gamma\n";
    ...
    #[test]
    fn diff_usfm_sources_works_with_preserve_and_merge_to_visible() {
        let baseline = "\\id GEN\n\\c 1\n\\p  \n\\v 1 Alpha\n";
        let current = "\\id GEN\n\\c 1\n\\p \n\\v 1 Alpha\n";

        let preserve = diff_usfm_sources(
            baseline,
            current,
            &TokenViewOptions::default(),
            &BuildSidBlocksOptions::default(),
        );
        let merged = diff_usfm_sources(
            baseline,
            current,
            &TokenViewOptions {
