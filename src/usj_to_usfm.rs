...
use crate::usj::roundtrip_fingerprint;
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UnexpectedValue {
                ...
                expected,
                actual,
            } => write!(f, "unexpected value for {context}: expected {expected}, got {actual}"),
...
fn extract_roundtrip_source(root: &Map<String, Value>, value: &Value) -> Option<String> {
    let metadata = root.get("_dovetail_roundtrip")?.as_object()?;
    ...
    map.remove("_dovetail_roundtrip");
...
mod tests {
    use super::*;
    use crate::{parse, to_usj_roundtrip_value, to_usj_value};
    use serde_json::json;
    ...
    fn writes_simple_document_from_usj() {
        ...
        assert!(usfm.contains("\\p\\v 1 In the beginning") || usfm.contains("\\p \\v 1 In the beginning"));
    }
    ...
    fn modified_roundtrip_usj_falls_back_to_synthesized_writer() {
        let source = "\\id GEN Genesis\n\\c 1\n\\p\n\\v 1 In the beginning\n";
        let handle = parse(source);
        let mut usj = to_usj_roundtrip_value(&handle);
