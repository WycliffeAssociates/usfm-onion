...
use crate::internal::recovery::RecoveryPayload;
use crate::internal::syntax::{Document, LeafKind, Node};
...
pub fn debug_dump(handle: &ParseHandle, options: DebugDumpOptions) -> String {
    ...
    let _ = writeln!(
        out,
        "recoveries: {}",
        crate::parse::handle::recoveries(handle).len()
    );
    ...
    if options.include_recoveries {
        let _ = writeln!(out, "\n[recoveries]");
        for (index, recovery) in crate::parse::handle::recoveries(handle).iter().enumerate() {
            if index >= options.limit {
                let _ = writeln!(
                    out,
                    "... {} more recoveries",
                    crate::parse::handle::recoveries(handle).len() - options.limit
                );
                break;
            }
