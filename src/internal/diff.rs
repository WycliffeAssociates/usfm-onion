...
//                         let original_text_only =
//                             strip_usfm_markers_for_display(&original.text_full);
//                         let current_text_only = strip_usfm_markers_for_display(&current.text_full);
//                         out.push(SidBlockDiff {
//                             block_id: id,
//                             semantic_sid: current.semantic_sid.clone(),
...
//         &baseline_tokens,
//         baseline.book_code().unwrap_or("unknown"),
//     );
//     let current_groups =
...
//         strip_all_whitespace(&original_text) == strip_all_whitespace(&current_text);
...
//         let modified = diffs
//             .iter()
//             .find(|diff| diff.status == DiffStatus::Modified)
//             .unwrap();
...
//         assert_eq!(
//             map.get("GEN").and_then(|chapters| chapters.get(&1)),
//             Some(&vec![1])
//         );
