use crate::format::{FormatOptions, FormattableFlatToken, format_tokens};
...
impl TokenFix {
    ...
    pub fn target_token_id(&self) -> &str {
        match self {
            TokenFix::ReplaceToken { target_token_id, .. }
            | TokenFix::InsertAfter { target_token_id, .. } => target_token_id,
        }
    }
}
