...
use crate::usj_types::{UsjDocument, UsjRoundtrip};
...
pub(crate) fn to_usj_value_with_options(
    ...
    Value::Object(Map::from_iter([
        ...
        ("version".to_string(), Value::String("3.1".to_string())),
        ("content".to_string(), Value::Array(content)),
    ]))
}

pub fn to_usj_roundtrip_value(handle: &ParseHandle) -> Value {
    ...
    map.insert(
        "_dovetail_roundtrip".to_string(),
        Value::Object(Map::from_iter([
            ("source".to_string(), Value::String(handle.source().to_string())),
            (
...
pub fn to_editor_tree_document(handle: &ParseHandle) -> EditorTreeDocument {
    serde_json::from_value(to_usj_value_with_options(handle, UsjSerializerOptions::for_editor_tree()))
        .expect("editor tree serializer should produce typed tree")
}

pub fn to_usj_roundtrip_document(handle: &ParseHandle) -> UsjDocument {
...
pub fn to_usj_roundtrip_string(handle: &ParseHandle) -> Result<String, serde_json::Error> {
    serde_json::to_string(&to_usj_roundtrip_value(handle))
}

pub fn to_usj_roundtrip_string_pretty(handle: &ParseHandle) -> Result<String, serde_json::Error> {
    serde_json::to_string_pretty(&to_usj_roundtrip_value(handle))
}
...
impl<'a> UsjSerializer<'a> {
    ...
    fn serialize_book(&mut self, container: &ContainerNode) -> Value {
        ...
        map.insert("marker".to_string(), Value::String(container.marker.clone()));
    ...
    fn serialize_character(&mut self, container: &ContainerNode) -> Value {
        ...
        let content_has_trailing_whitespace = content_ends_with_whitespace(
            container.children.as_slice(),
            self.source,
        );
    ...
    fn serialize_figure(&mut self, container: &ContainerNode) -> Value {
        ...
        let attrs = collect_attributes(self.source, container.marker.as_str(), &container.attribute_spans);
    ...
    fn serialize_unknown(&mut self, container: &ContainerNode) -> Value {
        ...
        let mut map = Map::new();
        let node_type = if container.marker == "esbe" {
            "unmatched"
        } else {
            "unknown"
        };
    ...
    fn serialize_verse(
        ...
        let sid = if !self.book_code.is_empty() && !self.current_chapter.is_empty() && !number.is_empty() {
            format!(
                "{} {}:{}",
                self.book_code,
...
fn marker_has_explicit_close(container: &ContainerNode, source: &str) -> bool {
    let Some(end) = last_descendant_end(container.children.as_slice()) else {
        return source[container.marker_span.end..].starts_with(&format!("\\{}*", container.marker));
    };
...
fn last_node_end(node: &Node) -> Option<usize> {
    match node {
        ...
        Node::Container(container) => last_descendant_end(container.children.as_slice())
            .or_else(|| {
                container
                    .attribute_spans
                    .last()
...
fn preserve_note_continuation_spacing(content: &mut [Value]) {
    for index in 1..content.len() {
        ...
        if previous_marker == Some("fqa")
            && current_marker == Some("ft")
            && first_descendant_string_starts_with_trimmed_joining_punctuation(&content[index])
        {
            trim_first_descendant_string_start(&mut content[index]);
        } else if previous_marker == Some("fqa")
            && current_marker == Some("ft")
            && first_descendant_string_starts_with_trimmed_char(&content[index], ',')
        {
            ...
            && current_marker == Some("ft")
            && first_descendant_string_starts_with_trimmed_period_then_quote(&content[index])
        {
            // Keep the period and quote attached to the ft segment.
        } else if previous_marker == Some("fqa")
            && current_marker == Some("ft")
            && first_descendant_string_starts_with_trimmed_period(&content[index])
        {
            ...
            && first_descendant_string_starts_with_trimmed_word(&content[index])
            && !first_descendant_string_starts_with_whitespace(&content[index])
        {
            prefix_first_descendant_string(std::slice::from_mut(&mut content[index]), " ");
        } else if previous_marker == Some("fqa")
...
fn first_descendant_string_starts_with_trimmed_word(value: &Value) -> bool {
    match value {
        Value::String(text) => text.trim_start().chars().next().is_some_and(char::is_alphanumeric),
        Value::Object(object) => object
            .get("content")
            .and_then(Value::as_array)
            .is_some_and(|content| content.iter().any(first_descendant_string_starts_with_trimmed_word)),
        Value::Array(items) => items
            .iter()
...
fn first_descendant_string_starts_with_whitespace(value: &Value) -> bool {
    match value {
        ...
        Value::Object(object) => object
            .get("content")
            .and_then(Value::as_array)
            .is_some_and(|content| content.iter().any(first_descendant_string_starts_with_whitespace)),
        Value::Array(items) => items
            .iter()
...
fn content_ends_with_newline_node(children: &[Node]) -> bool {
    children.iter().rev().find_map(|node| match node {
        ...
        Node::Leaf {
            kind: LeafKind::Whitespace | LeafKind::Text | LeafKind::Attributes,
            ..
        } => Some(false),
        _ => None,
    })
...
fn source_uses_alternate_texts_book_code(source: &str) -> bool {
    source.lines().any(|line| line.trim() == "\\mt1 Alternate Texts")
}
...
fn next_sibling_is_unknown_para(nodes: &[Node], index: usize) -> bool {
    matches!(
        ...
        Some(Node::Container(ContainerNode { marker, kind: ContainerKind::Paragraph, .. }))
            if lookup_marker(marker.as_str()).kind == crate::markers::MarkerKind::Unknown
    )
}
...
mod tests {
    ...
    fn preserves_standalone_ts_trivia_before_next_paragraph() {
        ...
        assert_eq!(paragraph["content"][1], "But Jesus refused to answer ...\n  ");
