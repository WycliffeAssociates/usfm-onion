...
use crate::markers::{default_attribute, lookup_marker};
use crate::recovery::{ParseRecovery, RecoveryCode, RecoveryPayload};
use crate::syntax::{ContainerKind, ContainerNode, LeafKind, Node};
...
struct UsxSerializer<'a> {
    source: &'a str,
    document: &'a crate::syntax::Document,
...
impl<'a> UsxSerializer<'a> {
    ...
    fn try_serialize_compacted_empty_intro_headers(
        ...
        if container.marker != "rem" || !children_are_effectively_empty(container.children.as_slice(), self.source) {
            return Ok(false);
        }
        ...
        self.writer.write_event(Event::Text(BytesText::new(&merged_markers)))?;
    ...
    fn try_serialize_table(&mut self, nodes: &[Node], index: &mut usize) -> Result<bool, UsxError> {
        let Some(Node::Container(ContainerNode { kind: ContainerKind::TableRow, .. })) = nodes.get(*index) else {
            return Ok(false);
        };
        ...
        for row_index in start..*index {
            let is_last_row = row_index + 1 == *index;
            let Node::Container(row) = &nodes[row_index] else {
                continue;
            };
        ...
        self.close_verse()?;
        self.writer.write_event(Event::End(BytesEnd::new("table")))?;
        Ok(true)
    }

    fn try_serialize_chapter(&mut self, nodes: &[Node], index: &mut usize) -> Result<bool, UsxError> {
        let Some(Node::Chapter { marker_span, number_span }) = nodes.get(*index) else {
            return Ok(false);
        };
        ...
        while let Some(node) = nodes.get(next) {
            match node {
                Node::Container(container) => match container.marker.as_str() {
                    "ca" => {
                        if let Some(text) = plain_text_from_nodes(container.children.as_slice(), self.source) {
                            altnumber = Some(text);
                            next += 1;
                        } else {
                            break;
        ...
        let sid = if !self.book_code.is_empty() && !number.is_empty() {
            Some(format!("{} {}", self.book_code, strip_leading_zeros(&number)))
    ...
    fn try_serialize_verse(&mut self, nodes: &[Node], index: &mut usize) -> Result<bool, UsxError> {
        let Some(Node::Verse { marker_span, number_span }) = nodes.get(*index) else {
            return Ok(false);
        };
    ...
    fn serialize_container(
        ...
        match container.kind {
            ...
            ContainerKind::Paragraph | ContainerKind::Header | ContainerKind::Meta | ContainerKind::Unknown => {
                if container.marker == "esbe" {
    ...
    fn serialize_para(
        ...
        if lookup_marker(container.marker.as_str()).kind == crate::markers::MarkerKind::Unknown {
            elem.push_attribute(("status", "unknown"));
        } else if container.marker == "s5" {
            elem.push_attribute(("status", "unknown"));
        } else if container_contains_verse(container.children.as_slice()) && self.current_chapter.is_empty() {
            elem.push_attribute(("status", "invalid"));
        } else if paragraph_is_invalid_for_current_position(
            container.marker.as_str(),
    ...
    fn serialize_char(&mut self, container: &ContainerNode) -> Result<(), UsxError> {
        ...
        if lookup_marker(container.marker.as_str()).valid_in_note && self.note_depth == 0 {
            elem.push_attribute(("status", "invalid"));
        } else if self.note_depth > 0 && marker_is_invalid_in_note_context(container.marker.as_str()) {
            elem.push_attribute(("status", "invalid"));
        }
        ...
        if children_are_effectively_empty(container.children.as_slice(), self.source) && raw_fallback.is_none() {
            self.writer.write_event(Event::Empty(elem))?;
            return Ok(());
        }
        self.writer.write_event(Event::Start(elem))?;
        ...
        if !trim_last && trailing_separator_after_last_content(container.children.as_slice(), self.source) {
    ...
    fn serialize_ref(&mut self, container: &ContainerNode) -> Result<(), UsxError> {
        ...
        for (key, value) in collect_attributes(self.source, container.marker.as_str(), &container.attribute_spans) {
            elem.push_attribute((key.as_str(), value.as_str()));
        }
        if children_are_effectively_empty(container.children.as_slice(), self.source) {
            self.writer.write_event(Event::Empty(elem))?;
    ...
    fn serialize_note(&mut self, container: &ContainerNode) -> Result<(), UsxError> {
        ...
        elem.push_attribute(("style", container.marker.as_str()));
        let (category, filtered) = extract_category_nodes(container.children.as_slice(), self.source);
        ...
        if normalized.iter().all(|node| is_ignorable_trivia_node(node, self.source)) {
            self.writer.write_event(Event::Empty(elem))?;
            return Ok(());
        }
        self.writer.write_event(Event::Start(elem))?;
    ...
    fn serialize_figure(&mut self, container: &ContainerNode) -> Result<(), UsxError> {
        ...
        if children_are_effectively_empty(container.children.as_slice(), self.source) {
            ...
            self.writer.write_event(Event::Start(elem))?;
            self.serialize_children(container.children.as_slice(), ContentTrim::inline())?;
            self.writer.write_event(Event::End(BytesEnd::new("figure")))?;
        }
        Ok(())
    }
    ...
    fn serialize_sidebar(&mut self, container: &ContainerNode) -> Result<(), UsxError> {
        ...
        if filtered.iter().all(|node| is_ignorable_trivia_node(node, self.source)) {
            self.writer.write_event(Event::Empty(elem))?;
        } else {
            self.writer.write_event(Event::Start(elem))?;
            self.serialize_children(filtered.as_slice(), ContentTrim::container())?;
            self.writer.write_event(Event::End(BytesEnd::new("sidebar")))?;
        }
    ...
    fn serialize_periph(&mut self, container: &ContainerNode) -> Result<(), UsxError> {
        ...
        if remaining.iter().all(|node| is_ignorable_trivia_node(node, self.source)) {
            self.writer.write_event(Event::Empty(elem))?;
        } else {
            self.writer.write_event(Event::Start(elem))?;
            self.serialize_children(remaining, ContentTrim::container())?;
            self.writer.write_event(Event::End(BytesEnd::new("periph")))?;
        }
        Ok(())
    }
    ...
    fn serialize_table_row(
        ...
        elem.push_attribute(("style", container.marker.as_str()));
        self.writer.write_event(Event::Start(elem))?;
        let last_cell_index = container
            .children
            .iter()
            .rposition(|node| matches!(
                node,
        ...
        for (index, child) in container.children.iter().enumerate() {
            ...
            if let Node::Container(cell) = child {
                if matches!(cell.kind, ContainerKind::TableCell) {
    ...
    fn note_was_unclosed(&self, marker_span: &std::ops::Range<usize>) -> bool {
        self.recoveries
            .iter()
            .any(|recovery| recovery.code == RecoveryCode::UnclosedNote && recovery.span == *marker_span)
    }
    ...
    fn keep_verse_open_after_para(&self, container: &ContainerNode, continues_verse: bool) -> bool {
        let paragraph_has_or_inherits_verse =
            self.current_verse_sid.is_some() || container_contains_verse(container.children.as_slice());
...
fn para_vid<'a>(
    ...
    } else if para_supports_vid(marker) {
        current_verse_sid
    } else if is_heading_bridge_marker(marker) && continues_verse {
        current_verse_sid
    } else {
        None
    }
...
fn children_are_effectively_empty(children: &[Node], source: &str) -> bool {
    children.iter().all(|node| is_ignorable_trivia_node(node, source))
}

fn container_contains_verse(children: &[Node]) -> bool {
    children.iter().any(|node| matches!(node, Node::Verse { .. }))
}
...
fn is_mergeable_empty_intro_header(container: &ContainerNode, source: &str) -> bool {
    children_are_effectively_empty(container.children.as_slice(), source)
        && matches!(container.marker.as_str(), "h" | "mt" | "mt1" | "mt2" | "mt3" | "mt4")
}
...
fn paragraph_is_invalid_for_current_position(
    ...
    if current_chapter.is_empty() {
        return matches!(
            marker,
            "p" | "m" | "po" | "pr" | "pc" | "pm" | "pmo" | "pmc" | "pmr" | "pi" | "pi1"
                | "pi2" | "pi3" | "nb" | "mi" | "cls" | "li" | "li1" | "li2" | "li3" | "li4"
                | "q" | "q1" | "q2" | "q3" | "q4" | "qm" | "qm1" | "qm2" | "qm3" | "lh"
                | "lf" | "b"
        );
    }
...
fn is_intro_paragraph(marker: &str) -> bool {
    matches!(
        ...
        "ip" | "ipi" | "im" | "imi" | "ipq" | "imq" | "ipr" | "iq" | "iq1" | "iq2" | "iq3"
            | "iot" | "io" | "io1" | "io2" | "io3" | "io4" | "ili" | "ili1" | "ili2" | "iex"
            | "ie"
    )
}

fn para_can_keep_verse_open(marker: &str) -> bool {
    para_supports_vid(marker) || marker == "lit" || is_heading_bridge_marker(marker)
}
...
fn find_unescaped_quote(value: &str) -> Option<usize> {
    ...
    (0..bytes.len()).find(|&index| bytes[index] == b'"' && (index == 0 || bytes[index - 1] != b'\\'))
}

fn resolve_default_attr_keys(marker: &str, attrs: Vec<(String, String)>) -> Vec<(String, String)> {
    let default_key = default_attribute(marker);
    attrs
        .into_iter()
        .map(|(key, value)| {
            if key == "default" {
...
fn normalize_note_child_spacing(nodes: &[Node], source: &str) -> Vec<Node> {
    ...
    while index < nodes.len() {
        ...
        if let Node::Container(container) = node {
            if matches!(container.kind, ContainerKind::Character) {
                let mut next = index + 1;
...
fn is_whitespace_only_leaf(node: &Node, source: &str) -> bool {
    match node {
        Node::Leaf {
            kind: LeafKind::Text | LeafKind::Whitespace | LeafKind::Newline,
            span,
        } => normalize_text(&source[span.clone()], false).trim().is_empty(),
        _ => false,
    }
}
...
fn content_ends_with_newline_node(children: &[Node]) -> bool {
    children
        .iter()
        .rfind(|node| !matches!(node, Node::Leaf { kind: LeafKind::Whitespace, .. }))
        .is_some_and(|node| matches!(node, Node::Leaf { kind: LeafKind::Newline, .. }))
}
...
fn normalize_preserved_raw_attributes(
    ...
    if marker == "w" && attr_gap_before {
        if let Some(default_key) = default_attribute(marker) {
            let prefixed = format!(" |{default_key}=\"");
            let unprefixed = format!("|{default_key}=\"");
            let stripped = text
                .strip_prefix(&prefixed)
                .map(|remainder| (" ", remainder))
                .or_else(|| text.strip_prefix(&unprefixed).map(|remainder| ("", remainder)));
