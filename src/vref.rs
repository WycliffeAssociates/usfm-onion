...
use crate::syntax::{ContainerKind, Node};
...
pub fn to_vref_map(handle: &ParseHandle) -> VrefMap {
    ...
    collect_nodes(handle.document().children.as_slice(), handle.source(), &mut state, &mut map);
    ...
    for node in nodes {
        match node {
            ...
            Node::Leaf { kind, span } if !state.current_ref.is_empty() => match kind {
                crate::syntax::LeafKind::Text | crate::syntax::LeafKind::Whitespace => {
                    state.current_text.push_str(&source[span.clone()]);
                }
                crate::syntax::LeafKind::Newline | crate::syntax::LeafKind::Attributes => {}
            },
            Node::Milestone { .. } => {}
            _ => {}
        }
    ...
    for node in nodes {
        if !seen_meaningful_child
            && matches!(
                node,
                Node::Leaf {
                    kind: crate::syntax::LeafKind::Whitespace | crate::syntax::LeafKind::Newline,
                    ..
                }
            )
        {
