...
pub fn from_usx_string(input: &str) -> Result<String, UsxToUsfmError> {
    ...
    serializer.serialize_children(root.children.as_slice())?;
...
impl UsxToUsfmSerializer {
    ...
    fn serialize_ms(&mut self, element: &XmlElement) -> Result<(), UsxToUsfmError> {
        ...
        if !attrs.is_empty() {
            self.output.push(' ');
            self.output.push_str(serialize_usfm_attrs(attrs.as_slice()).as_str());
        }
        self.output.push_str("\\*");
        self.at_line_start = false;
        Ok(())
...
fn collect_attrs<'a>(element: &'a XmlElement, excluded: &[&str]) -> Vec<(&'a str, &'a str)> {
    element
        ...
        .filter_map(|(key, value)| {
            if excluded.iter().any(|excluded_key| excluded_key == &key.as_str())
                || key == "sid"
                || key == "eid"
                || key == "vid"
            {
...
fn parse_xml(input: &str) -> Result<XmlElement, UsxToUsfmError> {
        ...
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Text(event)) => {
                let text = normalize_xml_text(
                    &event
                        .unescape()
                        .map_err(|error| UsxToUsfmError::Xml(error.to_string()))?
                        .into_owned(),
                );
                if text.is_empty() {
                    buf.clear();
                ...
                if let Some(parent) = stack.last_mut() {
                    ...
                    return Err(UsxToUsfmError::Xml("text node outside root element".to_string()));
                }
            }
            Ok(Event::CData(event)) => {
                ...
                if let Some(parent) = stack.last_mut() {
                    ...
                    return Err(UsxToUsfmError::Xml("CDATA node outside root element".to_string()));
                }
            }
            Ok(Event::End(_)) => {
                let Some(elem) = stack.pop() else {
                    return Err(UsxToUsfmError::Xml("unexpected closing tag".to_string()));
                };
                if let Some(parent) = stack.last_mut() {
                    parent.children.push(XmlNode::Element(elem));
                } else {
            ...
            Ok(Event::Eof) => break,
            Err(error) => return Err(UsxToUsfmError::Xml(error.to_string())),
        }
        buf.clear();
    }

    if !stack.is_empty() {
        return Err(UsxToUsfmError::Xml("unclosed element stack".to_string()));
    }

    root.ok_or_else(|| UsxToUsfmError::Xml("missing root element".to_string()))
}
...
fn parse_attrs(
    ...
    reader: &Reader<&[u8]>,
) -> Result<Vec<(String, String)>, UsxToUsfmError> {
    let mut attrs = Vec::new();
    for attr in event.attributes() {
        let attr = attr.map_err(|error| UsxToUsfmError::Xml(error.to_string()))?;
