use crate::handle::{ParseAnalysis, ParseHandle};
use crate::lexer::lex;
use crate::markers::{MarkerKind, lookup_marker};
use crate::recovery::{ParseRecovery, RecoveryCode};
use crate::syntax::{ContainerKind, ContainerNode, Document, LeafKind, Node};
use crate::token::{LexToken, RawTokenKind, normalized_marker_name, number_prefix_len};
...
impl ParserState {
    ...
    fn close_pending_milestone(&mut self, closed: bool) {
...
fn collect_analysis(tokens: &[LexToken]) -> ParseAnalysis {
    ...
    for (index, token) in tokens.iter().enumerate() {
        ...
        {
            state.close_pending_milestone(false);
        }

        if (state.pending_chapter.is_some() || state.pending_verse.is_some())
            && !matches!(
                token.kind,
                RawTokenKind::Whitespace | RawTokenKind::Newline | RawTokenKind::Text
            )
        {
        ...
        match token.kind {
            ...
            RawTokenKind::MilestoneEnd => {
                state.close_pending_milestone(true);
            }
            RawTokenKind::Text => {
            ...
            RawTokenKind::Attributes => handle_attributes(token, &mut state),
        }
    }
...
fn handle_open(index: usize, token: &LexToken, state: &mut ParserState) {
    let Some(marker) = token.marker_name() else {
        return;
    };
    let marker = normalized_marker_name(marker);

    if token.kind == RawTokenKind::Milestone {
    ...
    match info.kind {
        ...
        MarkerKind::Character => {
            if in_note_context(&state.stack)
                && token.kind != RawTokenKind::NestedMarker
...
fn handle_text(index: usize, token: &LexToken, state: &mut ParserState) {
    if state
        .stack
        .last()
...
fn handle_attributes(token: &LexToken, state: &mut ParserState) {
    if let Some(milestone) = state.pending_milestone.as_mut() {
        milestone.attribute_spans.push(token.span.clone());
        return;
    }
...
fn close_matching_marker(close_marker: &str, token: &LexToken, state: &mut ParserState) {
    ...
    loop {
        ...
        match note_idx {
            Some(idx) => {
                ...
                state.analysis.recoveries.push(ParseRecovery::marker_with_related(
...
fn close_character_in_note(state: &mut ParserState) {
    loop {
        match state.stack.last().map(|open| open.kind) {
            ...
            | Some(MarkerKind::Unknown)
            | Some(MarkerKind::Figure)
            | Some(MarkerKind::TableCell) => state.close_and_append_top(),
            _ => break,
        }
    }
}
...
fn has_open_paragraph(stack: &[OpenMarker]) -> bool {
    stack.iter().rev().any(|open| {
        matches!(
            open.kind,
            MarkerKind::Paragraph | MarkerKind::TableRow
        )
    })
}
...
fn unknown_marker_starts_new_block(state: &ParserState) -> bool {
    state
        ...
        .iter()
        .rev()
        .find(|open| matches!(open.kind, MarkerKind::Paragraph | MarkerKind::Header | MarkerKind::Meta))
        .is_some_and(|open| {
            open.children.iter().rev().find_map(|node| match node {
                Node::Leaf {
...
fn is_same_note_family(stack: &[OpenMarker], incoming_marker: &str) -> bool {
    let note_family = stack
        ...
        .and_then(|open| marker_family(&open.marker));
    let incoming_family = marker_family(incoming_marker);
...
fn should_close_current_note_char(
    ...
    let Some(current_char) = stack.iter().rev().find(|open| open.kind == MarkerKind::Character) else {
        return false;
    };
    ...
    if matches!(current_char.marker.as_str(), "ft" | "fk" | "xq") {
        return false;
    }

    current_info.valid_in_note && incoming_marker != "ref" && incoming_marker != "jmp"
}

fn marker_family(marker: &str) -> Option<char> {
    match marker {
        "f" | "fe" | "ef" => Some('f'),
        "x" | "ex" => Some('x'),
        _ => match marker.chars().next() {
            Some('f') => Some('f'),
            Some('x') => Some('x'),
            _ => None,
        },
...
fn token_is_milestone_attribute_continuation(token: &LexToken) -> bool {
    token.kind == RawTokenKind::Text && parses_as_attribute_continuation(token.text.as_str())
}
...
mod tests {
    use super::*;
    use crate::{RecoveryCode, recoveries};
        ...
        let milestone = handle
            ...
            .find_map(|node| match node {
                Node::Container(ContainerNode {
                    ...
                    Node::Milestone { attribute_spans, .. } => Some(attribute_spans),
                    _ => None,
                }),
                _ => None,
            })
            ...
            .find_map(|node| match node {
                Node::Container(ContainerNode {
                    ...
                    Node::Container(ContainerNode {
                        ...
                        Node::Container(ContainerNode { marker, children, .. }) if marker == "xt" => {
                            Some(children)
                        }
                        _ => None,
                    }),
